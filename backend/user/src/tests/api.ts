...
describe("/user/admin", () => {
  ...
  before(async () => {
    ...
    admin = await chai.request(app).post("/user/admin/admin/").set("content-type", "application/json").send({
      display_name: "admin",
  ...
  describe("POST /user", () => {
    it("should create a user", async () => {
      const res = await chai.request(app).post(`/user/admin/user/`).set("content-type", "application/json").send({
        display_name: "test",
  ...
  describe("POST /admin", () => {
    it("should create admin", async () => {
      const res = await chai.request(app).post("/user/admin/admin/").set("content-type", "application/json").send({
        display_name: "test",
        email: "test@email.com",
        password: "123456",
      });
  ...
  describe("POST /admin/login", () => {
    it("should login admin", async () => {
      const res = await chai.request(app).post("/user/admin/admin/login").set("content-type", "application/json").send({
        email: "test_admin@email.com",
        password: "123456",
      });
      expect(res).to.have.status(200);
...
describe("/user/api", () => {
  ...
  describe("POST /login", () => {
    it("should login user", async () => {
      const res = await chai.request(app).post("/user/api/login").set("content-type", "application/json").send({
        email: "test_user@email.com",
        password: "123456",
      });
      user = res.body;
  ...
  describe("GET /auth", () => {
    it("should authenticate", async () => {
      const res = await chai.request(app).get("/user/api/auth").set("Authorization", user.login_token);
      expect(res).to.have.status(200);
      expect(res.body).to.be.a("object");
      expect(res.body).to.have.property("data");
    });
    it("should not authenticate", async () => {
      const res = await chai.request(app).get("/user/api/auth").set("Authorization", "invalid_token");
