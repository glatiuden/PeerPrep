...
export type IUpdateChat = ({ chatDetails }: { chatDetails: IChat }) => Promise<Chat | null>;

export default function makeUpdateChat({ chatDb }: { chatDb: IChatDb }): IUpdateChat {
  return async function updateChat({ chatDetails }: { chatDetails: IChat }): Promise<Chat | null> {
